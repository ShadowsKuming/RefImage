...
def _build_system(project: dict, project_id: str) -> str:
    ...
    lines = [
        ...
        "- 如果话题跑偏，自然引导回拍摄规划。",
        "",
        "★★【设备/服装/道具的『已确认/准备好了』是用户自己在清单上点选的，你没有对应工具】：清单里每一项旁边"
        "都有一个用户自己点的确认勾选，这是他自己動手核对实物时用的，不归你管、也不存在让你去标记的字段。"
        "用户让你『把设备都标成已确认』这类请求时，别去改 add_equipment 的 name/desc（那样会把状态词焊进设备"
        "名称、把干净的名字搞乱，还容易搞出重复项），直接告诉他『这个我这边记不了，你在设备清单那栏自己点一下"
        "勾选就行』，不要调用任何工具。",
        "",
        ...
        "10. ★★【告别只说一次，别在『要不要结束』上循环】：用户说『结束了/不聊了/保存退出/晚安』这类明确收尾的话，"
        "你只回【一次】简短的送别（≤2句，别写成一段温情小作文），而且【别再给任何『保存退出/再确认一次/好的退出吧』"
        "这类还在邀请他继续聊『要不要结束』的选项】——这轮 options 就留空。用户后面又点了个告别类的话，也不要再触发"
        "新一轮送别、不要重复『谢谢你的信任』『好好休息』这类车轱辘话——安安静静收着就好。",
        "",
        "═══ 角色资料 ═══",
...
def _suggest_options(char_name: str, messages: list[dict], reply_lang: str = "zh") -> list[str]:
    ...
    system = (
        ...
        "不存在『还没定』这个答案。\n"
        "3.5 ★★ 如果最后一条是在【告别/收尾】（谢谢信任、好好休息、期待下次上线这类送别话，没有再问具体问题），"
        "就直接提交【空数组】——绝不能编『好的退出吧/保存了安心休息/再确认一次』这类选项，那等于在邀请用户继续聊"
        "『要不要结束』，会把道别变成循环。\n"
        "4. 和用户已经说过的保持一致，绝不矛盾。\n"
